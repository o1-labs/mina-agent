...
def measure(env, w: Workload, ref: str, *, symbol: str | None, repeats: int, out_dir: Path,
            run_dune, timeout_s: int) -> PerfRun:
    ...
    for _ in range(repeats):
        r = env.run([*_time_cmd(), *argv], capture=True, cwd=cwd, timeout=timeout_s)
    ...
    r = env.run(["env", "OCAMLRUNPARAM=v=0x400", *argv], capture=True, cwd=cwd, timeout=timeout_s)
    ...
    if symbol and tools_available()["samply"]:
        ...
        env.run(["samply", "record", "--save-only", "--unstable-presymbolicate", "-o", str(prof), "--", *argv],
...
def compare(env, g, manifest_tests, workload: str, base: str, head: str, *, symbol: str | None = None,
            repeats: int = 3, run_dune, timeout_s: int = 1800) -> PerfCompare:
    ...
    try:
        checkout(base_sha)
        b = measure(env, w, base, symbol=symbol, repeats=repeats, out_dir=out_dir, run_dune=run_dune, timeout_s=timeout_s)
        checkout(head_sha)
        h = measure(env, w, head, symbol=symbol, repeats=repeats, out_dir=out_dir, run_dune=run_dune, timeout_s=timeout_s)
    finally:
